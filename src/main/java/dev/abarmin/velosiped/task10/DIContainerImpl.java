...
public class DIContainerImpl implements DIContainer {
    ...
    @Override
    public void init() {
        String packageName = "dev.abarmin.velosiped.task10";
    ...
    private void collectControllerRoutings(Class aClass, Object newInstance) throws Exception {
        ...
        for (Method method : methods) {
            if (method.isAnnotationPresent(RequestMapping.class)) {
                ...
                List<ParameterExtractor> parameterExtractorList = new ArrayList<>();
                ...
                for (Parameter parameter : parameters) {
                    if (parameter.isAnnotationPresent(RequestBody.class)) {
                        ...
                        if (parameter.isAnnotationPresent(QueryParameter.class)) {
                            String uriParameterName = parameter.getAnnotation(QueryParameter.class).value();
                            QueryParameterExtractor queryParameterExtractor = new QueryParameterExtractor(uriParameterName, parameter.getType());
                            collectDependencies(QueryParameterExtractor.class, queryParameterExtractor);

                            parameterExtractorList.add(queryParameterExtractor);
                        }
                    }
                }
